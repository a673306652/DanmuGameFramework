...
public class Timer
{
    ...
    public static Timer Register(float duration, Action onComplete, Action<float> onUpdate = null,
        bool isLooped = false, bool useRealTime = false, MonoBehaviour autoDestroyOwner = null, float startingValue = 0)
    {
        ...
        Timer timer = new Timer(duration, onComplete, onUpdate, isLooped, useRealTime, autoDestroyOwner,startingValue);
    ...
    /// <summary>
    /// if timer is loop type, skip current loop and continue.
    /// </summary>
    public void SkipCurrentLoop()
    {
        if (isLooped)
        {
    ...
    private Timer(float duration, Action onComplete, Action<float> onUpdate,
        bool isLooped, bool usesRealTime, MonoBehaviour autoDestroyOwner,float startingValue = 0)
    {
        ...
        _AccTime = _PreTime = this.usesRealTime ? Time.realtimeSinceStartup : Time.time; 

        this._autoDestroyOwner = autoDestroyOwner;
        this._hasAutoDestroyOwner = autoDestroyOwner != null;

        this._startTime = this.GetWorldTime() + startingValue;
    ...
    {
        var current = this.usesRealTime ? Time.realtimeSinceStartup : Time.time;
         var offset = (current - _PreTime) * timeScale;
         _AccTime += offset;
         _PreTime = current;
         return _AccTime;
    }
