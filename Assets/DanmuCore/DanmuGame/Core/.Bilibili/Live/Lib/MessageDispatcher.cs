...
namespace BilibiliUtilities.Live.Lib
{
    public class MessageDispatcher : IMessageDispatcher
    {
        public async Task DispatchAsync(JObject message, IMessageHandler messageHandler)
        {
            try
            {
                var type = message["cmd"].ToString();
                //UnityEngine.Debug.Log(message);
                switch (type)
                {
                    case "POPULARITY_RED_POCKET_NEW"://多少人看过直播间
                        await messageHandler.RedPackageNewHandlerAsync(RedPackageNewMessage.JsonToRedPackageNewMessage(message));
                        break;
                    case "WATCHED_CHANGE"://多少人看过直播间
                        //UnityEngine.Debug.Log(message);
                        break;
                    case "DANMU_MSG":
                    ...
                    case "INTERACT_WORD":
                        ...
                        break;
                    case "ONLINE_RANK_V2": //更新排行榜信息
                        await messageHandler.RoomRankMessageHandlerAsync(RoomRankMessage.JsonToRoomRankMessage(message));
                        //UnityEngine.Debug.Log(message);
                        break;
