    ...
    public abstract class LeanMethod : MonoBehaviour
    {
        ...
        public static float Smooth(LeanEase ease, float x)
        {
            switch (ease)
            {
                ...
                case LeanEase.BounceInOut:
                    return x < 0.5f ? 0.5f - SmoothBounce(1 - x * 2) / 2 : 0.5f + SmoothBounce(x * 2 - 1) / 2;
                case LeanEase.GameView_In_Curve: return VFX_StaticCurveManager.GetGameviewInCurve().Evaluate(x);
                case LeanEase.GameView_Out_Curve: return VFX_StaticCurveManager.GetGameviewOutCurve().Evaluate(x);
                case LeanEase.BusinessView_In: return VFX_StaticCurveManager.GetBVInCurve().Evaluate(x);
                case LeanEase.BusinessView_Btn_In: return VFX_StaticCurveManager.GetBVBtnInCurve().Evaluate(x);
                case LeanEase.BusinessView_DelayShow: return VFX_StaticCurveManager.GetBVDelayShowCurve().Evaluate(x);
                case LeanEase.BusinessView_DelayClose: return VFX_StaticCurveManager.GetBVDelayCloseCurve().Evaluate(x);
                case LeanEase.BusinessView_Out : return VFX_StaticCurveManager.GetBVOutCurve().Evaluate(x);
            }
...
namespace Lean.Transition.Editor
{
    ...
    public class LeanMethod_Editor : LeanEditor
    {
        ...
        private void DrawTargetAlias(SerializedProperty sTarget, SerializedProperty sAlias,
            System.Reflection.FieldInfo data)
        {
            TARGET tgt;
            TARGET[] tgts;
            ...
            var rect = Reserve();
            var rectF = rect;
            rectF.xMin += EditorGUIUtility.labelWidth - 50;
            rectF.width = 48;
            var rectM = rect;
            rectM.xMin += EditorGUIUtility.labelWidth;
            var rectL = rectM;
            rectL.xMax -= rectL.width / 2 + 1;
            var rectR = rectM;
            rectR.xMin += rectR.width / 2 + 1;
            ...
            if (data != null)
            {
                ...
                if (state != null && state.CanFill > -1)
                {
                    ...
                    if (GUI.Button(rectC,
                            new GUIContent("fill", "Copy the current value from the Target into this component?"),
            ...
            if (setCount == 0 || setCount == 2)
            {
                EditorGUI.LabelField(rect,
                    new GUIContent("Target : Alias",
                        "Target = The component or object that will be modified by this transition\n\nAlias = If this transition is on a prefab, this allows you to give a name to the component or object that will be modified by this transition."));
            }
            else if (setTarget == true)
            {
                EditorGUI.LabelField(rect,
                    new GUIContent("Target", "The component or object that will be modified by this transition"));
            }
            else
            {
                EditorGUI.LabelField(rect,
                    new GUIContent("Alias",
                        "If this transition is on a prefab, this allows you to give a name to the component or object that will be modified by this transition."));
            }
            ...
            if (setAlias == true)
            {
                ...
                foreach (var method in tgt.GetComponents<LeanMethodWithStateAndTarget>())
                {
                    ...
                    if (methodTargetType != expectedType)
                    {
                        ...
                        {
                            if (methodTargetType.IsSubclassOf(typeof(Component)) == true &&
                                expectedType.IsSubclassOf(typeof(Component)) == true)
                            {
                                continue;
                            }

                            Error(
                                "This alias is used by multiple transitions. This only works if they all transition the same type (e.g. Transform.localPosition & Transform.localScale both transition Transform).");
        ...
        private void DrawAutoFill(TARGET tgt, System.Reflection.FieldInfo data)
        {
            ...
            if (state != null && state.CanFill > -1)
            {
                ...
                BeginDisabled(state.CanFill == 0);
                if (GUI.Button(rect,
                        new GUIContent("auto fill", "Copy the current value from the scene into this component?"),
                        EditorStyles.miniButton) == true)
                {
                    state.Fill();
                }
        ...
        protected override void OnInspector()
        {
            ...
            if (dataProperty != null)
            {
                ...
                while (true)
                {
                    if (dataProperty.name != "Duration" && dataProperty.name != "Target" &&
                        dataProperty.name != "TargetAlias")
                    {
                        if (dataProperty.propertyType == SerializedPropertyType.Quaternion)
                        {
                            ...
                            var eulerAngles = EditorGUILayout.Vector3Field(
                                new GUIContent(dataProperty.displayName, dataProperty.tooltip),
                                dataProperty.quaternionValue.eulerAngles);
